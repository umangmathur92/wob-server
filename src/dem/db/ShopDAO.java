				...
				for (String item_id : tempList) {
					...
					if (data != null) {
						...
						if (type.equals("Animal") || type.equals("Plant")) {
							item = new ShopItem(data.getID(), itemLevel, data.getName(), data.getDescription(),
									data.getCost());
							...
							item.setExtraArgs(Arrays.asList(String.valueOf((int) data.getBiomass()),
									String.valueOf(data.getDietType()), String.valueOf(data.getTrophicLevel()),
									predatorStr, preyStr));
