...
public class GameDB {
	...
	private GameDB() {
		...
		String connectURI = String.format("jdbc:mysql://%s/%s?user=%s&password=%s", Configuration.DBURL,
				Configuration.DBName, Configuration.DBUsername, Configuration.DBPassword);

		dataSource = ConnectionPool.setupDataSource(connectURI);
	}

	/**
	 * Parses the database configuration file and store those values into memory.
	 */
