...
public final class ConsumeDAO {
	...
	public static Map<Integer, List<Integer>> getPreyToPredatorTable(int eco_type) throws SQLException {
		 ...
		 * because it can cause problems when parsing predator lists; NOTE: this is
		 * preventing the PREDATOR from being hidden
		 */
		...
		String query = "SELECT c.* FROM `consume` c "
				+ "JOIN `eco_type_species` ets ON c.`species_id` = ets.`species_id` " + "WHERE ets.`eco_type` = "
				+ eco_type + " " + " ORDER BY c.`species_id`";
	...
	public static Map<Integer, List<Integer>> getPredatorToPreyTable(int eco_type) throws SQLException {
		...
		/*
		 * 4/25/14, JTC, had to add join to prevent hidden species from being read in
		 * because it can cause problems when parsing prey lists; NOTE: this is
		 * preventing the PREY from being hidden
		 */
		// 10/1/14, JTC, "hidden" functionality replaced by ets table

		String query = "SELECT c.* FROM `consume` c " + "JOIN `eco_type_species` ets ON c.`prey_id` = ets.`species_id` "
				+ "WHERE ets.`eco_type` = " + eco_type + " " + " ORDER BY c.`species_id`";
		// String query = "SELECT c.*, s.`hidden` FROM `consume` c JOIN `species` s "
		// + "ON c.`prey_id` = s.`species_id` "
		// + "WHERE s.`hidden` = " + String.valueOf(NOT_HIDDEN) + " ORDER BY
		// c.`species_id`";
