...
public class SpeciesType {
	...
	/*
	 * 4/21/14, JTC, new parameter lists needed for testing simulations only loaded
	 * for simulations run from SimJobMenu; added here due to similarity to
	 * nodeDistribution setup
	 */
	...
	/**
	 * getPredatorNodeIDs duplicates intent of getPredatorIndex, but currently that
	 * methods contents are commented out and it is still actively called by
	 * SimEngine. Do not want to disrupt. Creating getPredatorNodeIDs in its place.
	 * 4/22/14, JTC
	 *
	...
	public List<Integer> getPredatorNodeIDs() {
		...
		for (Integer speciesId : predatorList) {
			...
			List<Integer> spNodeList = ServerResources.getSpeciesTable().getSpecies(speciesId).getNodeList();
	...
	/**
	 * getPreyNodeIDs duplicates intent of getPreyIndex, but currently that methods
	 * contents are commented out and it is still actively called by SimEngine. Do
	 * not want to disrupt. Creating getPreyNodeIDs in its place. 4/22/14, JTC
	 *
	...
	/**
	 * Load list of species IDs from database; static call - data is not stored in
	 * object. Specifies SQL "WHERE" clause to get specific species. Only used in
	 * Sim_Job environment. JTC 4/2014 9/13/14 - JTC - added param eco_type.
	 *
	 * @param whereClause
	 * @return
	 */
