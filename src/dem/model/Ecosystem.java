...
public class Ecosystem {
	...
	// Node ID -> Count
	// private final Map<Integer, Integer> nodeList = new HashMap<Integer,
	// Integer>();
	// Node ID -> Biomass
	...
	// 5/5/14, JTC, new hashmap to store persistent lists of specieszonetypes for
	// each player
	// in order to store game or player changes to species parameters (I'm certain
	// that there
	// is a lot of unnecessary redundancy here, but I don't have time to look into
	// it)
	...
	private int scoreSmoothingWindowSize = Constants.DEFAULT_SCORE_SMOOTHING_WINDOW_SIZE; // the number of days of raw
																							// score history to use for
																							// smoothing
	...
	/**
	 * Calculate a version of the smoothed environment score with numbers in a
	 * smaller range more interpretable by players.
	 * 
	...
	/**
	 * Calculate a weighted moving average of the rawEnvironmentScore based on the
	 * history of raw environment scores over the past [scoreSmoothingWindowSize]
	 * game days.
	 * 
	...
	/**
	 * Update the in-memory raw environment score history window. Does not update
	 * the database.
	 */
	...
	/**
	 * Set the current game day. Intended for testing purposes, not general usage.
	 */
	...
	/**
	 * Calculate the environment score for the current point in time, without
	 * smoothing.
	 * 
	...
	/**
	 * Calculate a weighted total biomass of all species in the ecosystem, where the
	 * weight for a species is its trophic level.
	 * 
	...
	/**
	 * Calculate the biomass-based Shannon index of the ecosystem. The Shannon index
	 * is a standard measure of ecological diversity. This version uses biomass to
	 * represent population, rather than the number of individuals, and uses base-2
	 * logarithms.
	 * 
	...
	public void updateScore() {
		 ...
		 * 
		 * int lastMonth = Integer.valueOf(rowFirst.get(rowFirst.size() - 1)); int
		 * currentMonth = gameEngine.getCurrentMonth();
		 * Log.println("after currentMonth");
		 * 
		 * for (int i = lastMonth; i <= currentMonth; i++) { if (i == lastMonth && i ==
		 * currentMonth) { rowSecond.set(i, Integer.toString(score)); } else if (i !=
		 * lastMonth) { rowFirst.add(Integer.toString(i));
		 * rowSecond.add(Integer.toString(score)); } } Log.println("after for loop");
		 * 
		 * String csv = CSVParser.createCSV(score_csv); Log.println("after CSVParser");
		 * CSVDAO.createScoreCSV(eco_id, csv);
		 * 
		 * Log.println("In EcoSystem, before ResponseChart()"); Log.println("csv = " +
		 * csv); if(!Constants.DEBUG_MODE){ ResponseChart response = new
		 * ResponseChart(); response.setType(2); response.setCSV(csv);
	...
	/**
	 * Calculate the current and high environment scores, updating the database and
	 * sending the new score values to the client.
	 */
	public void updateEnvironmentScore() {
		...
		if (!Constants.DEBUG_MODE) {
			...
			ResponseUpdateEnvironmentScore response = new ResponseUpdateEnvironmentScore(eco_id, score, highEnvScore);
	...
	// if (animal.getID() != organism.getID() && (organism.getOrganismType() ==
	// Constants.ORGANISM_TYPE_PLANT || animal.getHungerLevel() < 1.0f)) {
	// int[] preyList = animal.getSpeciesType().getPreyIDs();
	//
	// if (preyList != null) {
	// for (int prey_id : preyList) {
	// if (prey_id == species_id) {
	// double temp = Math.sqrt(Math.pow(animal.getX() - organism.getX(), 2) +
	// Math.pow(animal.getY() - organism.getY(), 2));
	//
	...
	// * @param species_id
	// */
	// public void setNodeParam(World world, int param, double value, int
	// species_id) {
	...
	// zoneNodes.modifyNodeBiomassByFraction(fraction,
	...
	// zoneNodes.modifyNodeBiomassByFraction(fraction,
	// SpeciesZoneType.SpeciesTypeEnum.PLANT);
	// }
	//
	// /**
	...
	// * @param species_id
	// */
	// public void modifyNodeBiomassByFraction(World world, double fraction, int
	// species_id) {
	...
	// zoneNodes.modifyNodeBiomassByAmount(amount,
	// SpeciesZoneType.SpeciesTypeEnum.ANIMAL);
	// }
	//
	// /**
	...
	// * @param species_id
	// */
	// public void modifyNodeBiomassByAmount(World world, double amount, int
	// species_id) {
	...
	/**
	 * Calculate a weighted average of the given values, where the weights follow a
	 * linear scale such that the first element is given the smallest weight and the
	 * last element is given the largest weight.
	 */
