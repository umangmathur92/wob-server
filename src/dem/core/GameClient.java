...
public class GameClient {
	...
	// Responses
	private final List<GameResponse> responses = Collections.synchronizedList(new ArrayList<GameResponse>()); // Temporarily
																												// store
																												// responses
																												// for
																												// client
	...
	/**
	 * Initialize the GameClient using the client socket and creating both input and
	 * output streams.
	 *
	 * @param session_id
	 *            holds the unique identifier of this session
	 * @param clientSocket
	...
	/**
	 * Holds the main loop that processes incoming requests by first identifying its
	 * type, then interpret the following data in each determined request class.
	 * Queued up responses created from each request class will be sent after the
	 * request is finished processing.
	 *
	 * The loop exits whenever the isPlaying flag is set to false. One of these
	 * occurrences is triggered by a timeout. A timeout occurs whenever no activity
	 * is picked up from the client such as being disconnected.
	 */
	public void run() {
		try {
			...
			if (size > 0) {
				...
				if (counter > 1) {
					Log.printf("Note, network latency issue identified, wait count = %d, protocol ID = %d", counter,
							request_id);
				}
	...
	public void select(Player player) {
		...
		if (player.getLastPlayed() == null) {
			...
			Ecosystem ecosystem = EcosystemController.createEcosystem(world_id, player.getID(),
					player.getName() + "'s Ecosystem", (short) type);
	 ...
	 * stored for the player will be saved into the database and any ties with the
	...
	private void DebugPacket(byte[] bytes, int id, boolean outbound) {
		int limit = 20;
		Log.printf("\nGameClient.DebugPacket(), %s:%d, client: %s, 1st %d bytes out of %d", (outbound ? "SEND" : "RCV"),
				id, (account == null ? "n/a" : account.getUsername()), limit, bytes.length);
		for (int i = 0; i < Math.min(bytes.length, limit); i++) {
			byte[] b = new byte[1];
			b[0] = bytes[i];
			Log.printf("byte %d: hx %02x, dc %02d, asc '%s'", i, b[0] & 0x000000FF, b[0] & 0x000000FF, new String(b));
		}
