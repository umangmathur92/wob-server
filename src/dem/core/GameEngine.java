		...
		for (Entry<Integer, Integer> entry : speciesList.entrySet()) {
			...
			if (ecosystem.containsSpecies(species_id)) {
				...
				for (SpeciesGroup group : species.getGroups().values()) {
					...
					EcoSpeciesDAO.updateBiomass(eco_id, group.getID(), species_id,
							(biomassPrev + biomass) / species.getGroups().size());
					if (!Constants.DEBUG_MODE) {
						ResponseSpeciesCreate response = new ResponseSpeciesCreate(Constants.CREATE_STATUS_DEFAULT,
								ecosystem.getID(), group);
		...
		for (Entry<Integer, Integer> entry : speciesListForRemoval.entrySet()) {
			...
			if (ecosystem.containsSpecies(species_id)) {
				...
				for (SpeciesGroup group : species.getGroups().values()) {
					...
					group.setBiomass(0);
					if (!Constants.DEBUG_MODE) {
						ResponseSpeciesCreate response = new ResponseSpeciesCreate(Constants.REMOVE_STATUS_DEFAULT,
								ecosystem.getID(), group);
						NetworkFunctions.sendToLobby(response, lobby.getID());
					}
				}
